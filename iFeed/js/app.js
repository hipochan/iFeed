        ...
        var App = (function () {
            function App() {
                ...
                this.initialize = function () {
                    ...
                    $('#addFeed').on('show.bs.modal', function () { return _this.openAddFeedWindow(); });
                    $('#addFeed').on('shown.bs.modal', function () { return _this.openedAddFeedWindow(); });
                    $('#addFeed').on('hidden.bs.modal', function () {
                        setTimeout(function () { return $('#add_feed').blur(); }, 10);
                    });
                    ...
                    $('#add_feed_window_submit').on('click', function () { return _this.addFeedSubmit(); });
                    $('#content').html('');
                    $('#content').sortable({
                        handle: '.feed-title',
                        update: function (event, ui) { return _this.feedSorted(); }
                    });
                    $('#search_text').autocomplete({
                        source: function (request, response) {
                            ...
                            $.ajax({
                                ...
                                dataType: "json",
                                type: "GET",
                                success: function (data) { return response(data[1]); }
                            });
                        },
                        delay: 300
                    });
                ...
                this.escapeHTMLString = function (value) {
                    value = value.replace(/<("[^"]*"|'[^']*'|[^'">])*>/g, '');
                    return $('<div />').text(value).html();
                };
                this.escapeHTML = function (value) {
                    value = _this.escapeHTMLString(value);
                ...
                this.getFeedResponse = function (message) {
                    ...
                    if (link !== undefined && link != '') {
                        feedHTML += '		<div class="feed-title"><a href="' + _this.escapeHTMLString(link).replace(/\"/g, '\\"') + '" target="_blank">' + _this.escapeHTML(title) + '</a></div>\n';
                    }
                    else {
                    ...
                    $(item).each(function () {
                        ...
                        var link = self.escapeHTMLString(item.link).replace(/\"/g, '\\"');
                        if (item.description !== '') {
